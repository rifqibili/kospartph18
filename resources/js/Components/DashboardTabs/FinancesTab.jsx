                                ...
                                {visibleFinances.length === 0 ? (
                                    ...
                                    <tr key={f.id} className="">
                                        ...
                                        <td className="p-3 text-slate-600">
                                            ...
                                            {f.receipt_path && (() => {
                                                // Bangun URL yang benar apapun format yang tersimpan di DB:
                                                // - path relatif baru: "receipts/file.jpg" → "/storage/receipts/file.jpg"
                                                // - lama dengan Storage::url: "/storage/receipts/file.jpg" → pakai langsung
                                                // - sudah full URL: "http://..." → pakai langsung
                                                let receiptUrl = f.receipt_path;
                                                if (!receiptUrl.startsWith('http') && !receiptUrl.startsWith('/')) {
                                                    receiptUrl = `/storage/${receiptUrl}`;
                                                }
                                                return (
                                                    <a href={receiptUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline mt-1 inline-flex items-center gap-1">
